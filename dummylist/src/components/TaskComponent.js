...
export default function TaskComponent({description, alternativeText = false,  membersCompletedTask, totalMembersInChallenge, onPress, isCompleted = false}) 
{
    return (
        <View style={[style.taskContainer, {height: 94, alignSelf: "center", backgroundColor: !isCompleted ? "#001D34" : "#F2B705" }]}>
            <View style={style.taskTextContainer}>
                ...
                <Text style={style.taskSmallText}> {alternativeText ? alternativeText : `Done by ${membersCompletedTask.length}/${totalMembersInChallenge} people`}</Text>
            </View>
            {!isCompleted ? (
